...
ROUTES: tuple[Route, ...] = (
    ...
    Route("GET", "/api/v1/repo", handlers.v1_repo, needs_slug=True),
    # Making one. The service has existed since companies could be versioned at all; what was
    # missing was the route, so the only way to start was a terminal.
    Route("POST", "/api/v1/repo", handlers.v1_repo_create, needs_slug=True),
