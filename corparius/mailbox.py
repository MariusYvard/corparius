...
def diagnosis(exc: Exception, lang="en") -> str:
    ...
    if isinstance(exc, imaplib.IMAP4.error):
        ...
        if "folder" in low or "nonexistent" in low or "select" in low:
            return p(
                f"The folder '{cfg.get('CORP_IMAP_FOLDER', 'INBOX')}' does not exist on "
        ...
        return text or p(
            "The IMAP server refused the request.", "Le serveur IMAP a refusé la requête."
        )
    ...
    if isinstance(exc, ssl.SSLError):
        return p(
            f"TLS handshake failed on port {port}. Port 993 expects implicit TLS, "
...
def check(timeout: int = 20, lang="en") -> dict:
    ...
    if not configured():
        return {
            ...
            "configured": False,
            "detail": p(
                "No IMAP server set, so inbox triage keeps using its mock.",
    ...
    try:
        ...
        if typ != "OK":
            return {
                ...
                "configured": True,
                "detail": p(
                    f"Connected, but the folder '{folder}' could not be opened.",
        ...
        return {
            ...
            "unread": unread,
            "detail": p(
                f"Connected to {host}, folder {folder}: {unread} unread. "
