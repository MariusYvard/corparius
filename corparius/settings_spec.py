...
GROUPS: list[dict] = [
    {
        "name": "access",
        "label_en": "Console access",
        "label_fr": "Accès à la console",
    ...
    {
        "name": "inference",
        "label_en": "Local inference",
        "label_fr": "Inférence locale",
    ...
    {
        "name": "safety",
        "label_en": "Safety and cadence",
        "label_fr": "Sécurité et cadence",
    ...
    {
        "name": "mail",
        "label_en": "Mail account",
        "label_fr": "Compte mail",
        "warn": True,
        "preset": True,
        "test": "mail",
    ...
    {
        "name": "leads",
        "label_en": "Leads and signals",
        "label_fr": "Prospects et signaux",
...
MAIL_PRESETS: list[dict] = [
    ...
    {
        "id": "scaleway",
        "label": "Scaleway TEM (sending only)",
...
WARN_EN = (
    "These keys authorise real-world effects: Stripe reports real money, SMTP "
...
WARN_FR = (
    "Ces clés autorisent des effets réels : Stripe rapporte de l'argent réel, "
...
SPEC: list[FieldSpec] = [
    ...
    _f(
        "CORP_PLUGINS_ENABLED",
        "access",
        type="bool",
        default="false",
        label_en="Enable plugins",
        label_fr="Activer les plugins",
    ...
    _f(
        "CORP_PLUGINS",
        "access",
        label_en="Enabled plugins",
        label_fr="Plugins activés",
    ...
    _f(
        "CORP_PLUGINS_ALLOW_UNVERIFIED",
        "access",
        type="bool",
        default="false",
        label_en="Allow unverified plugins",
        label_fr="Autoriser les plugins non vérifiés",
    ...
    _f(
        ...
        label_fr="Vérifier les mises à jour",
    ...
    _f(
        ...
        type="int",
        default="420",
        label_en="Timeout (seconds)",
        label_fr="Délai (secondes)",
    ...
    _f(
        ...
        type="int",
        default="10000",
        label_en="Tokens per minute",
        label_fr="Tokens par minute",
    ...
    _f(
        "CORP_LOOP_SIMILARITY_THRESHOLD",
        "safety",
        type="float",
        default="0.95",
        label_en="Loop similarity threshold",
        label_fr="Seuil de similarité du garde-boucle",
    ...
    _f(
        ...
        type="int",
        default="2",
        label_en="Max identical tool calls",
        label_fr="Appels d'outil identiques max",
    ),
    _f(
        "CORP_HITL_TOOLS",
        "safety",
        default="send_financial_transaction,publish_production_code,deploy_site",
        label_en="Tools that need approval",
        label_fr="Outils exigeant une approbation",
    ...
    _f(
        ...
        type="int",
        default="4",
        label_en="WIP limit per agent",
        label_fr="Limite d'en-cours par agent",
    ...
    _f(
        "CORP_CEO_APPROVE_CAP",
        "safety",
        type="int",
        default="3",
        label_en="CEO approvals per review",
        label_fr="Approbations CEO par revue",
    ),
    _f(
        ...
        type="int",
        default="20",
        label_en="Outreach emails per run",
        label_fr="Emails d'outreach par run",
    ),
    # --- payments -----------------------------------------------------------
    _f(
        "STRIPE_API_KEY",
        ...
        type="password",
        secret=True,
        label_en="Stripe API key",
        label_fr="Clé API Stripe",
    ...
    _f(
        ...
        type="password",
        secret=True,
        label_en="App password",
        label_fr="Mot de passe d'application",
    ...
    _f(
        ...
        "mail",
        label_en="Send tests and fallbacks to",
        label_fr="Envoyer tests et replis à",
    ...
    _f(
        ...
        default="INBOX",
        advanced=True,
        label_en="Folder to read",
        label_fr="Dossier à lire",
    ...
    _f(
        "CORP_OUTREACH_DAILY_CAP",
        "mail",
        type="int",
        default="0",
        advanced=True,
        label_en="Daily send cap",
        label_fr="Plafond d'envoi quotidien",
    ...
    _f(
        ...
        "mail",
        advanced=True,
        label_en="Suppression list",
        label_fr="Liste de suppression",
    ...
    _f(
        ...
        "publishing",
        label_en="Local directory",
        label_fr="Dossier local",
    ...
    _f(
        "CORP_S3_ENDPOINT",
        "publishing",
        label_en="S3 endpoint",
        label_fr="Endpoint S3",
    ...
    _f(
        ...
        type="password",
        secret=True,
        label_en="S3 access key",
        label_fr="Clé d'accès S3",
    ),
    _f(
        "CORP_S3_SECRET",
        "publishing",
        type="password",
        secret=True,
        label_en="S3 secret key",
        label_fr="Clé secrète S3",
    ),
    _f(
        "CORP_DEPLOY_SSH_TARGET",
        "publishing",
        label_en="rsync target",
        label_fr="Cible rsync",
    ...
    _f(
        "CORP_LEADS_URL",
        "leads",
        label_en="Lead search URL",
        label_fr="URL de recherche",
    ...
    _f(
        ...
        label_en="Browser user agent",
        label_fr="User agent du navigateur",
    ),
    _f("CORP_ENRICHERS", "leads", default="local", label_en="Enrichers", label_fr="Enrichisseurs"),
    _f(
        "CORP_ENRICH_DOMAIN",
        "leads",
        label_en="Enrichment domain",
        label_fr="Domaine d'enrichissement",
    ...
    _f(
        ...
        "leads",
        default="browser,local",
        label_en="Signal sources",
        label_fr="Sources de signaux",
    ),
    _f(
        "CORP_SIGNALS_FILE",
        "leads",
        label_en="Local signals file",
...
SECRETS: set[str] = (
    {f.key for f in SPEC if f.secret}
    | {spec["key_env"] for spec in OPENAI_COMPAT_PROVIDERS.values()}
    | {"ANTHROPIC_API_KEY"}
)
...
def describe(key: str, lang_fields: bool = True) -> dict:
    ...
    if lang_fields:
        out.update(
            {
                "label_en": spec.label_en or key,
                "label_fr": spec.label_fr or key,
