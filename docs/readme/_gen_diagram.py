...
DARK = dict(
    bg0="#081A3E", bg1="#061232", card="#0A1D48", cardln="#254387", dots="#537CD6",
    text="#EFF4FC", muted="#9AA5BB", kicker="#8CCBFF", accent="#2456D3", warm="#EB933B",
    warmln="#7A4E1E", warmfill="#1C1608", teal="#64B8D2", arrow="#537CD6", dotop="0.16",
)
LIGHT = dict(
    bg0="#FFFFFF", bg1="#EEF2F9", card="#FFFFFF", cardln="#CAD1DF", dots="#B7C2D8",
    text="#142752", muted="#475572", kicker="#2456D3", accent="#2456D3", warm="#AA5900",
    warmln="#E0C79A", warmfill="#FBF3E4", teal="#2E7D96", arrow="#9AA9C6", dotop="0.5",
)
...
def chip(x, y, label, p):
    w = 10 + len(label) * 7.4
    return (f'<rect x="{x}" y="{y}" width="{w:.0f}" height="24" rx="12" fill="none" stroke="{p["cardln"]}" stroke-width="1"/>'
            f'<text x="{x+w/2:.0f}" y="{y+16}" text-anchor="middle" font-family="ui-monospace,Menlo,Consolas,monospace" font-size="11.5" fill="{p["muted"]}">{label}</text>'), w
...
'''
    ...
    s += card(xs[1], y2, CW, CH, p, "Human gate", "money / prod code wait for you", accent=p["warm"])
