...
resources; corparius/kernel/paths.py routes all writable state (the SQLite store, .env, the
...
# corparius modules are imported lazily in several places (orchestrator, sitegen,
# deploy, backup, update_check...), so pull the whole package in explicitly.
hiddenimports = collect_submodules("corparius")
