    ...
    "summary": frozenset(
        {
            ...
            "onboarding",
            # What the operator should do next, with a destination for each. In `summary` for the same
            # reason `onboarding` is: it is four short rows and both tabs that want it already poll
            # this resource.
            "next_steps",
            "permission_mode",
            "proposals_need_you",
...
def test_build_is_the_declared_union_and_nothing_more(a_company):
    ...
    # 34: `next_steps`. The CEO tab could answer questions and could not answer *the* question — an
    # operator who does not know what to do has nothing to type. These are derived from the store, not
    # generated by a model, because a plausible list is the worst possible answer here: it can name a
    # tab that does not apply or miss the two approvals actually holding the company up.
    assert len(declared) == 34, "the payload gained or lost a key; say which in DECLARED"
