...
exhaustive. `routes.ALL_ROUTES` is now the name for the whole surface, and that check uses it.

Stage 6 moved the table from `webui.py` to `api/routes.py`, and this file's first version read
`Path("corparius/webui.py")` — a path, which a move turns into a `FileNotFoundError` at best and
an empty scan at worst. It now reads the **module object**, so "every function of `handlers` is
in the table" is exact: there is no filter to fall out of date and nothing to point at the wrong
file. Three tests in this suite were reading a path when the last move happened and it broke all
three; that is the second time, and this is the fix that ends it.

The prefix that made a handler recognisable is gone with the move. In a module whose every
function is a handler, `_route_` was the module's own name repeated 57 times — and the check
below is stronger for it: no name pattern to match, just "defined here" against "in the table".
"""
...
from corparius.api import handlers, routes


def _defined() -> set[str]:
    """Every function `api/handlers.py` defines.

    From the module, not from a source path, and `__module__` is what makes it exact: the file
    imports `adapters` and `state`, and without that filter their functions would count as
    handlers the moment anyone imported one by name.
    """
    return {
        name
        for name, obj in vars(handlers).items()
        if inspect.isfunction(obj) and obj.__module__ == handlers.__name__
    }


def _reached() -> set[str]:
    return {route.handler.__name__ for route in routes.ALL_ROUTES}
...
def test_there_is_a_table_to_check():
    ...
    assert len(routes.ALL_ROUTES) >= 50, f"only {len(routes.ALL_ROUTES)} routes found"
    assert len(_defined()) >= 50, "the handler scan found almost nothing"
    # And they are the same size, which is what "both ends" means once the scan is exact.
    assert len(_defined()) == len(routes.ALL_ROUTES)
...
def test_the_canonical_table_is_the_two_tables():
    ...
    assert set(routes.ALL_ROUTES) == set(routes.ROUTES) | set(routes.PREFIX_ROUTES)
    assert len(routes.ALL_ROUTES) == len(routes.ROUTES) + len(routes.PREFIX_ROUTES)
...
def test_no_two_routes_answer_to_the_same_method_and_path():
    """A duplicate is not two routes: `routes._EXACT` is a dict, so the second silently replaces the
    first, and the handler that loses is unreachable while still looking wired."""
    keys = [(r.method, r.path) for r in routes.ALL_ROUTES]
...
def test_no_prefix_route_shadows_an_exact_one():
    """The reason there are two tables. `routes.match` checks exact first, so this cannot currently
    happen — and stating it is what keeps a future reordering from making it possible."""
    exact = {(r.method, r.path) for r in routes.ROUTES}
    for prefix in routes.PREFIX_ROUTES:
...
@pytest.mark.parametrize("route", sorted(routes.ALL_ROUTES, key=lambda r: (r.method, r.path)))
...
def test_no_route_declares_a_company_it_never_reads(route):
    ...
    `needs_slug` makes `server.Handler._dispatch` resolve the company and fall through to 404 when none is
    ...
    **The other direction is deliberately not asserted here**, and this is the audit so nobody
    ...
    them is guarded — either the value only ever becomes a SQL parameter (`handlers.chat_post`
    down to `store.status(slug)`, where a bogus slug returns empty rows) or the guard is one
    call away (`handlers.company_delete` -> `adapters.delete_company`, which starts with
    `slug not in state.companies()`; `handlers.deploy_post` -> `adapters.deploy` ->
    `state.load_company`). Checking
