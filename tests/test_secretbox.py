...
def test_store_encrypts_secret_values_at_rest(monkeypatch, tmp_path):
    ...
    raw = {
        r["key"]: r["value"] for r in store.db.execute("SELECT key, value FROM settings").fetchall()
    }
...
def test_store_plaintext_when_no_key(monkeypatch, tmp_path):
    ...
    raw = store.db.execute("SELECT value FROM settings WHERE key='ANTHROPIC_API_KEY'").fetchone()[0]
